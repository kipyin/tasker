"""Route `.msg` attachments into configured project buckets."""
...
def _load_task_ref(
    *,
    ...
    if not (task.project_id or "").strip():
        msg = "Task has no project_id; run `tasker classify` (or set a project) first."
        raise RoutingError(msg)
...
def route_task_attachments(
    *,
    ...
    """
    Extract attachments from the task's `.msg`, match routing rules, move into buckets.

    Writes JSONL events to ``<home>/logs/routing.jsonl``. Updates
    ``task.attachment_routes_json`` when not ``dry_run``.

    ``msg_opener`` defaults to ``extract_msg.openMsg``; inject a callable for tests.
    """
    ...
    with ctx as msg:
        ...
        if stored_names and len(stored_names) != len(attachments):
            msg_err = (
                f"Attachment count mismatch: .msg has {len(attachments)} attachments "
                f"but the database snapshot lists {len(stored_names)}."
            )
            raise RoutingError(msg_err)

        for index, att in enumerate(attachments):
            if stored_names:
                logical = sanitize_attachment_filename(stored_names[index])
            else:
                raw = getattr(att, "name", None) or ""
                logical = sanitize_attachment_filename(
                    str(raw).strip() or f"file-{index}"
                )
            ...
            with tempfile.TemporaryDirectory() as td:
                td_path = Path(td)
                src = _export_attachment_to_scratch(
                    att,
                    scratch_dir=td_path,
